...
public class XPA extends MFSK {
	...
	public void setState(int state) {
		...
		if (state == 1)
			...
			theApp.setStatusLabel("Decoding");
		else if (state == 5)
			theApp.setStatusLabel("Complete");
	}
	...
	private String startToneHunt(CircularDataBuffer circBuf, WaveData waveData) {
		...
		if ((difference < (toneDIFFERENCE - ErrorALLOWANCE) || (difference > (toneDIFFERENCE + ErrorALLOWANCE))))
			return null;
		// Tones found
		// Calculate the long error correction factor
		correctionFactor = LowTONE - tone1;
		// Tell the user
		line = theApp.getTimeStamp() + " XPA Start Tones Found (correcting by " + Integer.toString(correctionFactor)
				+ " Hz)";
		return line;
	}
	...
	private String getChar(int tone, String prevChar) {
		...
		if ((tone > (520 - errorAllowance)) && (tone < (520 + errorAllowance)))
			return ("Start Low");
		else if ((tone >= (600 - errorAllowance)) && (tone < (600 + errorAllowance)))
			return ("Sync Low");
		else if ((tone >= (680 - errorAllowance)) && (tone < (680 + errorAllowance)))
			return (" ");
		else if ((tone >= (720 - errorAllowance)) && (tone < (720 + errorAllowance)))
			return ("End Tone");
		else if ((tone >= (760 - errorAllowance)) && (tone < (760 + errorAllowance)))
			return ("0");
		else if ((tone >= (800 - errorAllowance)) && (tone < (800 + errorAllowance)))
			return ("1");
		else if ((tone >= (840 - errorAllowance)) && (tone < (840 + errorAllowance)))
			return ("2");
		else if ((tone >= (880 - errorAllowance)) && (tone < (880 + errorAllowance)))
			return ("3");
		else if ((tone >= (920 - errorAllowance)) && (tone < (920 + errorAllowance)))
			return ("4");
		else if ((tone >= (960 - errorAllowance)) && (tone < (960 + errorAllowance)))
			return ("5");
		else if ((tone >= (1000 - errorAllowance)) && (tone < (1000 + errorAllowance)))
			return ("6");
		else if ((tone >= (1040 - errorAllowance)) && (tone < (1040 + errorAllowance)))
			return ("7");
		else if ((tone >= (1080 - errorAllowance)) && (tone < (1080 + errorAllowance)))
			return ("8");
			...
			if (prevChar == "Sync Low")
				...
				return ("9");
		} else if ((tone >= (1160 - errorAllowance)) && (tone < (1160 + errorAllowance)))
			return ("Message Start");
		else if ((tone >= (1200 - errorAllowance)) && (tone < (1200 + errorAllowance)))
			return ("R");
		else if ((tone >= (1280 - errorAllowance)) && (tone < (1280 + errorAllowance)))
			return ("Start High");
		else
			return ("UNID");
	}
