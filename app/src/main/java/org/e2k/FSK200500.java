...
public class FSK200500 extends FSK {
				...
				if (ibit == 2) {
					...
					if (theApp.isDebug() == true) {
						...
						// CR
							...
							if (lineBuffer.lastIndexOf("162)5761") != -1)
								characterCount = MAXCHARLENGTH;
							// Improve the formatting of messages which contain traffic
							if ((lineBuffer.length() > 20) && (lineBuffer.charAt(lineBuffer.length() - 6) == '='))
								characterCount = MAXCHARLENGTH;
							if ((lineBuffer.length() > 20) && (lineBuffer.charAt(lineBuffer.length() - 6) == ')')
									&& (lineBuffer.charAt(lineBuffer.length() - 7) == ' '))
								characterCount = MAXCHARLENGTH;
						}
					}
					if (bcount != 7) {
	...
	private int fsk200500FreqHalf(CircularDataBuffer circBuf, WaveData waveData, int pos) {
		...
		if (high1 == high2) {
			...
			if (checkValid() == true) {
				...
				else if ((early[0] + late[0]) > (early[1] + late[1]))
					v = 1;
				else
					v = 0;
			} else {
	...
	private boolean checkValid() {
		if ((inChar[0] == false) && (inChar[6] == true) && (bcount >= 7))
			return true;
		else
			return false;
	}
	...
	public String getQuailty() {
		String line = "Missing characters made up " + String.format("%.2f", errorPercentage) + "% of this message ("
				+ Long.toString(missingCharCounter) + " characters missing)";
		return line;
	}
