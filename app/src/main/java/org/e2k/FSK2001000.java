...
public class FSK2001000 extends FSK {
	...
	public void setState(int state) {
		...
		if (state == 1)
			...
			theApp.setStatusLabel("Debug Only");
	}
	...
	public boolean decode(CircularDataBuffer circBuf, WaveData waveData) {
		...
		// Hunt for the sync sequence
			...
			String dout;
			if (sampleCount > 0)
				dout = syncSequenceHunt(circBuf, waveData);
			else
				dout = null;
			if (dout != null) {
		...
		// Main message decode section
			...
			if (symbolCounter >= samplesPerSymbol) {
				...
				// Compare the first 32 bits of the circular buffer to the known FSK200/1000
				// header
				...
				if (difSync < 2)
					processBlock();
				// If there have been more than 2880 bits with a header (i.e 10 blocks) we have
				// a serious problem
				if (bitsSinceLastBlockHeader > 2880)
					setState(1);
			}
		}
		// Debug only
	...
	private String syncSequenceHunt(CircularDataBuffer circBuf, WaveData waveData) {
		...
		if (theApp.isDebug() == true)
			line = line + " (lowBin=" + Integer.toString(lowBin) + " highBin=" + Integer.toString(highBin) + ")";
		return line;
	}
	...
	public String getQuailty() {
		String line = "There were " + Integer.toString(blockCount) + " blocks in this message with "
				+ Integer.toString(missingBlockCount) + " missing.";
		return line;
	}
	...
	private int compareSync(String comp) {
		...
		// If the input String isn't the same length as the SYNC String then we have a
		// serious problem !
		if (comp.length() != SYNC.length())
			return 32;
		int a, dif = 0, invdif = 0;
		for (a = 0; a < comp.length(); a++) {
			if (comp.charAt(a) != SYNC.charAt(a))
				dif++;
			if (comp.charAt(a) != INVSYNC.charAt(a))
				invdif++;
		}
		// If the inverted difference is less than 2 the user must have things the wrong
		// way around
		if (invdif < 2) {
	...
	private void processBlock() {
		...
		if (bitCount > 288)
			missingBlockCount = missingBlockCount + (bitCount / 288);
		...
		if (checkDividerBlock(data) == true) {
			...
			if (lineNos == 0) {
				// Display the total number of blocks which is encoded into block 0 bits
				// 64,65,66,67,80,81,82
				messageTotalBlockCount = ((data[8] & 240) >> 1) + ((data[10] & 224) >> 5) + 1;
				linesOut[0] = linesOut[0] + " : Total Message Size " + Integer.toString(messageTotalBlockCount)
						+ " blocks";
				// Display the number of messages contained
				int mcount = (data[12] & 240) >> 4;
				if (mcount == 1)
					linesOut[0] = linesOut[0] + " : This transmission contains one message.";
				else
					linesOut[0] = linesOut[0] + " : This transmission contains " + Integer.toString(mcount)
							+ " messages.";
			}
			// If block 1 display the block 1 special information
			else if (lineNos == 1) {
				linesOut[0] = linesOut[0] + " " + extractAddressee(data) + " " + extractDate(data) + " "
						+ extractMsgNumber(data) + extractMsgType(data) + extractGroupCount(data);
		...
		if (divider == false)
			...
			theApp.writeLine(linesOut[0], Color.RED, theApp.boldFont);
	...
	private String extractBlock1Mys(int da[]) {
		...
		// Display these
		String r = String.format("%05d", w) + " " + String.format("%05d", e) + " " + String.format("%05d", g) + " "
				+ String.format("%05d", h) + " " + String.format("%05d", j) + " " + String.format("%05d", k) + " "
				+ String.format("%05d", l) + " " + String.format("%05d", m);
		return r;
	}
	...
	private String extractDate(int d[]) {
		...
		if (dval == 1)
			...
			return " : 11th of month";
		else if (dval == 12)
			return " : 12th of month";
		else if (dval == 13)
			return " : 13th of month";
		else if (dval == 14)
			return " : 14th of month";
		else if (dval == 15)
			return " : 15th of month";
		else if (dval == 16)
			return " : 16th of month";
		else if (dval == 17)
			return " : 17th of month";
		else if (dval == 18)
			return " : 18th of month";
		else if (dval == 19)
			return " : 19th of month";
		else if (dval == 20)
			return " : 20th of month";
		else if (dval == 21)
			return " : 21st of month";
		else if (dval == 22)
			return " : 22nd of month";
		else if (dval == 23)
			return " : 23rd of month";
		else if (dval == 24)
			return " : 24th of month";
		else if (dval == 25)
			return " : 25th of month";
		else if (dval == 26)
			return " : 26th of month";
		else if (dval == 27)
			return " : 27th of month";
		else if (dval == 28)
			return " : 28th of month";
		else if (dval == 29)
			return " : 29th of month";
		else if (dval == 30)
			return " : 30th of month";
		else if (dval == 31)
			return " : 31st of month";
		else
			return " : Corrupt Date";
