...
public class XPA2 extends MFSK {
	...
	public boolean decode(CircularDataBuffer circBuf, WaveData waveData) {
		...
		if (state == 1) {
			...
			if ((sampleCount >= 0) && (sampleCount % 200 == 0))
	...
	private String getChar(int tone, String prevChar) {
		...
		if ((tone >= (978 - errorAllowance)) && (tone < (978 + errorAllowance)))
			return (" ");
		else if ((tone >= (1010 - errorAllowance)) && (tone < (1010 + errorAllowance)))
			return ("Sync Low");
		else if ((tone >= (1026 - errorAllowance)) && (tone < (1026 + errorAllowance)))
			return ("End Tone");
		// Allow for 1041 Hz and 1057 Hz Sync Highs
			...
			if (prevChar == "Sync Low")
				...
				return "R";
		} else if ((tone >= (1073 - errorAllowance)) && (tone < (1073 + errorAllowance)))
			return ("0");
		else if ((tone >= (1088 - errorAllowance)) && (tone < (1088 + errorAllowance)))
			return ("1");
		else if ((tone >= (1104 - errorAllowance)) && (tone < (1104 + errorAllowance)))
			return ("2");
		else if ((tone >= (1119 - errorAllowance)) && (tone < (1119 + errorAllowance)))
			return ("3");
		else if ((tone >= (1134 - errorAllowance)) && (tone < (1134 + errorAllowance)))
			return ("4");
		else if ((tone >= (1151 - errorAllowance)) && (tone < (1151 + errorAllowance)))
			return ("5");
		else if ((tone >= (1167 - errorAllowance)) && (tone < (1167 + errorAllowance)))
			return ("6");
		else if ((tone >= (1182 - errorAllowance)) && (tone < (1182 + errorAllowance)))
			return ("7");
		else if ((tone >= (1197 - errorAllowance)) && (tone < (1197 + errorAllowance)))
			return ("8");
		else if ((tone >= (1212 - errorAllowance)) && (tone < (1212 + errorAllowance)))
			return ("9");
		else
			return ("UNID");
	}
	...
	private void displayMessage(int freq) {
		...
		if ((tChar != "Sync High") && (tChar != "Sync Low") && (tChar != "End Tone") && (tChar != "UNID")
				&& (tChar != "")) {
	...
	private String startToneHunt(CircularDataBuffer circBuf, WaveData waveData) {
		...
		// Tell the user
		String line = theApp.getTimeStamp() + " XPA2 Start Tones Found (correcting by "
				+ Integer.toString(correctionFactor) + " Hz)";
		return line;
	}
	...
	private int xpa2Freq(CircularDataBuffer circBuf, WaveData waveData, int pos) {
		if (waveData.getSampleRate() == 8000.0) {
			...
			if (theApp.isInvertSignal() == false)
				...
				freq = PIVOT - freq + correctionFactor;
			return freq;
