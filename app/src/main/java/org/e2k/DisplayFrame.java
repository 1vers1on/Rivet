...
public class DisplayFrame extends JFrame implements ActionListener {
	...
	public DisplayFrame(String title, Rivet theApp) {
		...
		viewMenu.add(
				DisplayBad_item = new JRadioButtonMenuItem("Display Possible Bad Data", theApp.isDisplayBadPackets()));
		...
		viewMenu.add(freeChannelMarkerGW_item = new JRadioButtonMenuItem("View GW Free Channel Markers",
				theApp.isViewGWChannelMarkers()));
	...
	public void actionPerformed(ActionEvent event) {
		...
		for (int a = 0; a < theApp.MODENAMES.length; a++) {
			if (event_name == theApp.MODENAMES[a])
				theApp.setSystem(a);
		}
		// Load a WAV file
		...
		if (event_name == "Soundcard Input") {
			if (theApp.isSoundCardInput() == true)
				theApp.setSoundCardInput(false);
			else
				theApp.setSoundCardInput(true);
		}
		...
		if (event_name == "Display Possible Bad Data") {
			if (theApp.isDisplayBadPackets() == true)
				theApp.setDisplayBadPackets(false);
			else
				theApp.setDisplayBadPackets(true);
		}
		// View GW Free Channel Markers
		if (event_name == "View GW Free Channel Markers") {
			if (theApp.isViewGWChannelMarkers() == true)
				theApp.setViewGWChannelMarkers(false);
			else
				theApp.setViewGWChannelMarkers(true);
		}
		// Show UTC Time
		if (event_name == "Display UTC Time") {
			if (theApp.isLogInUTC() == true)
				theApp.setLogInUTC(false);
			else
				theApp.setLogInUTC(true);
		}
		// Exit
		if (event_name == "Exit") {
		...
		for (a = 0; a < trigList.size(); a++) {
			if (event_name.equals(trigList.get(a).getTriggerDescription() + trigList.get(a).getTypeDescription())) {
				...
				if (trigList.get(a).isActive() == true)
	...
	public void closeLogFile() {
		...
		try {
			...
			if (theApp.isGW() == true)
				theApp.fileWriteLine(theApp.getShipList());
			// Close the file
			theApp.file.flush();
			theApp.file.close();
		} catch (Exception e) {
	...
	// Display a dialog box so the user can select a location and name for a log
	// file
	...
	// Display a dialog box so the user can select a location and name for a bit
	// stream file
	...
	void DialogTriggerModify() {
		...
		// Check if any changes were made to the triggers and if so update the menu
		// items
		if (triggerModify.isChangedTriggers() == true)
			updateTriggerMenuItems();
	}
	...
	void updateTriggerMenuItems() {
		...
		int a;
		for (a = 0; a < trigList.size(); a++) {
			JMenuItem tmenu = new JRadioButtonMenuItem(
					trigList.get(a).getTriggerDescription() + trigList.get(a).getTypeDescription(),
					trigList.get(a).isActive());
