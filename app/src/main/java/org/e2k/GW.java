...
public class GW extends FSK {
		...
		if (state == 0) {
			...
			if (symbolCounter >= samplesPerSymbol100) {
				...
				if (theApp.isDebug() == true) {
					if (ibit == true)
						theApp.writeChar("1", Color.BLACK, theApp.boldFont);
					else
						theApp.writeChar("0", Color.BLACK, theApp.boldFont);
					characterCount++;
	...
	public void setState(int state) {
		...
		if (state == 1)
			...
			theApp.setStatusLabel("Msg Hunt");
	}
	...
	// that allows us to use the data for the early/late gate and to detect a half
	// bit (which is used as a stop bit)
	private boolean gwFreqHalf(CircularDataBuffer circBuf, WaveData waveData, int pos) {
		...
		if ((early[0] + late[0]) > (early[1] + late[1]))
			addToAdjBuffer(getPercentageDifference(early[0], late[0]));
		else
			addToAdjBuffer(getPercentageDifference(early[1], late[1]));
		...
		if (theApp.isBitStreamOut() == true) {
			if (out == true)
				theApp.bitStreamWrite("1");
			else
				theApp.bitStreamWrite("0");
		}
		return out;
	}
	...
	private void addToAdjBuffer(double in) {
		...
		if (in > 45.0) {
			...
			adjCounter++;
			if (adjCounter == adjBuffer.length)
				adjCounter = 0;
		}
	}
	...
	private boolean syncSequenceHunt(CircularDataBuffer circBuf, WaveData waveData) {
		...
		b0 = getFreqBin();
		// Check this first tone isn't just noise the highest bin must make up 10% of
		// the total
		if (getPercentageOfTotal() < 10.0)
			return false;
		pos = (int) samplesPerSymbol100 * 1;
		int f1 = getSymbolFreq(circBuf, waveData, pos);
		b1 = getFreqBin();
		// Check this second tone isn't just noise the highest bin must make up 10% of
		// the total
		if (getPercentageOfTotal() < 10.0)
			return false;
		if (f0 == f1)
		...
		// If either the low bin or the high bin are zero there is a problem so return
		// false
		if ((lowBin == 0) || (highBin == 0))
			return false;
		// The shift for GW FSK should be should be 200 Hz
		if ((highTone - lowTone) != 200)
			return false;
		else
			return true;
	}
	...
	private void processGWData() {
		...
		if (bitCount > 144) {
			...
			if ((frame.get(8).equals(0xf2)) && (frame.get(9).equals(0xf2)) && (frame.get(10).equals(0xf2))
					&& (frame.get(11).equals(frame.get(12))) && (frame.get(12).equals(frame.get(13)))
					&& (frame.get(13).equals(frame.get(14))) && (frame.get(14).equals(frame.get(15)))
					&& (frame.get(15).equals(frame.get(16))) && (frame.get(17).equals(0xff))) {
				StringBuilder lo = new StringBuilder();
				lo.append(theApp.getTimeStamp());
				lo.append(" GW Free Channel Marker from Station Code 0x" + Integer.toHexString(frame.get(12)) + " ("
						+ stationName(frame.get(12)) + ") ");
				int a;
				for (a = 0; a < 18; a++) {
					lo.append(" " + Integer.toHexString(frame.get(a)) + " ");
				}
				if (theApp.isViewGWChannelMarkers() == true)
			...
			if (sData.charAt(13) == '1')
				subType++;
			// Setup the display
			lo.append(" Type=" + Integer.toString(type) + " Count=" + Integer.toString(packetCounter) + " Subtype="
					+ Integer.toString(subType) + " (");
			// Display the header as binary
			lo.append(dataBitSet.extractSectionFromStart(0, 14));
			lo.append(") (" + displayGWAsHex(0) + ")");

			// If we have been in receiving position report for over 60 seconds it is never
			// going to come so reset
			if (receivingPositionReport == true) {
				long difTime = (System.currentTimeMillis() / 1000) - fragmentStartTime;
				// Only do this if positionFragmentCounter>0 to preven this appearing when
				// monitoring
				// shore side GW broadcasts
				if ((difTime > 60) && (positionFragmentCounter > 0)) {
					String line = theApp.getTimeStamp() + " position report timeout (fragment Count is "
							+ Integer.toString(positionFragmentCounter) + ")";
			...
			// An ongoing position report
				...
				else if (curFrag.equals(lastPositionFragment))
					return;
			...
			// Type 2 Subtype 101
				...
				if ((mInts.get(0) == 0xe2) && (mInts.get(1) == 0x72) && (mInts.get(2) == 0xff) && (mInts.get(3) == 0xff)
						&& (mInts.get(4) == 0xe7) && (mInts.get(5) == 0xe6)) {
			...
			if (theApp.isDisplayBadPackets() == true)
				theApp.writeLine(lo.toString(), Color.RED, theApp.boldFont);
			return;
		}
		// Handle very short packets
		// These packets are only 8 bits long and may be ACKs
		// Reports suggest they are 10010101 (0x95) but we need to see if this is always
		// the case
	...
	private String getGWChar(int c) {
		if (c == 0x60)
			...
			return "9";
		else if ((c == 0x78) || (c == 0x3e) || (c == 0x4e) || (c == 0xf8))
			return "<";
		else if ((c == 0x2e) || (c == 0x7c))
			return ",";
		else if (c == 0x5c)
			return ".";
		else if (c == 0x74)
			...
			return "A";
		else if ((c == 0x47) || (c == 0x3f))
			return "B";
		else if ((c == 0x37) || (c == 0x07))
			return "E";
		else if (c == 0x17)
			return "G";
		else if ((c == 0x7f) || (c == 0x63))
			return "L";
		else if (c == 0x5f)
			return "N";
		else if ((c == 0x1f) || (c == 0x43))
			return "O";
		else if (c == 0x13)
			return "W";
		else
	...
	// Decode the 2/101 packets contents into a MMSI and see if we have any details
	// of this ship
	...
	public String displayGW_ShipMMSI(List<Integer> mm, int totalDigits) {
		...
		for (a = 0; a < 6; a++) {
			...
			if (a < 5)
				followingNibble = (mm.get(a + 1) & 240) >> 4;
			else
				followingNibble = 0;
			boolean alternate;
			// Low nibble
			// If the nibble following the low nibble (which is in the next byte) is 0x8 or
			// greater
			// then we use the alternate numbering method
			if (followingNibble >= 0x8)
				alternate = true;
			else
				alternate = false;
			...
			if (digitCounter == totalDigits)
				return sb.toString();
			// High nibble
			// If the nibble following the high nibble (which is the low nibble) is 0x8 or
			// greater
	...
	// Big thanks to Alan W for all his help working out the encoding method used
	// here
	...
	private void createPositionReportLine() {
		if (receivingPositionReport == false)
			return;
		else
			receivingPositionReport = false;
		...
		String curFrag = displayGWAsAscii(0);
		if (curFrag.equals(lastPositionFragment))
			return;
		// It isn't so add this to the position report StringBuilder
		else
			positionReport.append(curFrag);
	...
	public String displayGW_ShoreMMSI(List<Integer> mm, int totalDigits) {
		...
		for (a = 0; a < 6; a++) {
			...
			boolean alternate;
			// Low nibble
			// If the following nibble (which is in the high nibble of the next byte) is
			// less than 0x8 then we use the alternate numbering method
			if (followingNibble < 0x8)
				alternate = true;
			else
				alternate = false;
			...
			if (digitCounter == totalDigits)
				return sb.toString();
			// High nibble
			// If the following nibble (which is in the low nibble) is less than 0x8 then we
			// use the alternate numbering method
			if (ln < 0x8)
				alternate = true;
			else
				alternate = false;
