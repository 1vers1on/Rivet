...
public class AT3x04 extends OFDM {
	...
	public void setState(int state) {
		this.state = state;
		if (state == 0)
			theApp.setStatusLabel("Setup");
		else if (state == 1)
			theApp.setStatusLabel("Signal Hunt");
		else if (state == 2)
	...
	public boolean decode(CircularDataBuffer circBuf, WaveData waveData) {
		...
		if (state == 0) {
			...
			//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			theApp.writeLine("Please note that this mode is experimental and doesn't work yet !", Color.RED,
					theApp.italicFont);
			//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			setState(1);
			return true;
		...
		// Look for the 12 carriers from this mode
			...
			if (sampleCount % 100 == 0) {
				...
				// Collect three lots of carrier info lists searching between 2700 Hz and 3500
				// Hz
				if (startCarrierCounter == 0) {
					startCarrierList1 = findOFDMCarriersWithinRange(spr, waveData.getSampleRate(), RDFT_FFT_SIZE, 0.8,
							270, 350);
					startCarrierCounter++;
				} else if (startCarrierCounter == 1) {
					startCarrierList2 = findOFDMCarriersWithinRange(spr, waveData.getSampleRate(), RDFT_FFT_SIZE, 0.8,
							270, 350);
					startCarrierCounter++;
				} else if (startCarrierCounter == 2) {
					startCarrierList3 = findOFDMCarriersWithinRange(spr, waveData.getSampleRate(), RDFT_FFT_SIZE, 0.8,
							270, 350);
					startCarrierCounter++;
					...
					if (AT3x04PilotToneHunt(spr) == true) {
						...
						if (AT3x04CarrierConfirm(clist) == true) {
							...
							sb.append(theApp.getTimeStamp() + " AT3x04 Pilot Tone found at " + Double.toString(toneFreq)
									+ " Hz");
	...
	private boolean AT3x04PilotToneHunt(double spectrum[]) {
		int a, pbin;
		// Look if a particular bin in startCarrierList1 also exists in
		// startCarrierList2 and startCarrierList3
	...
	private void addToTimingBuffer(double in) {
		...
		timingBufferCounter++;
		if (timingBufferCounter == TIMINGBUFFERSIZE)
			timingBufferCounter = 0;
	}
