...
public class RDFT extends OFDM {
		...
		// Look for the constant 8 carriers that signal a RDFT start
			...
			if (sampleCount % 50 == 0) {
				...
				if (RDFTCheck(clist) == true) {
					...
					sb.append(theApp.getTimeStamp() + " RDFT lead in tones found. Carrier 1 at "
							+ Double.toString(clist.get(0).getFrequencyHZ()) + " Hz");
			...
			if (pastEnergyBufferCounter == pastEnergyBuffer.length)
				pastEnergyBufferCounter = 0;
	...
	private boolean RDFTCheck(List<CarrierInfo> carrierList) {
		...
		// Check the difference between the highest carrier bin and the lowest is within
		// an allowable range
		int totalDifference = carrierList.get(7).getBinFFT() - carrierList.get(0).getBinFFT();
		if ((totalDifference < 159) || (totalDifference > 162))
			return false;
		// Check the average spacing of the carriers is more than 190 Hz and less than
		// 250 Hz
