...
public class RTTY extends FSK {
	...
	public void setState(int state) {
		...
		if (state == 1)
			...
			theApp.setStatusLabel("Decoding Traffic");
	}
			...
			if (symbolCounter >= samplesPerSymbol) {
				...
				if (theApp.isBitStreamOut() == true) {
					if (ibit == 1)
						...
						theApp.bitStreamWrite("2");
					else if (ibit == 3)
						theApp.bitStreamWrite("3");
				}
				...
				if (symbolTotal < previousSymbolTotal)
					oldSymbolPercentage[0] = 100.0 - ((symbolTotal / previousSymbolTotal) * 100.0);
				else
					oldSymbolPercentage[0] = 100.0 - ((previousSymbolTotal / symbolTotal) * 100.0);
				double av = (oldSymbolPercentage[0] + oldSymbolPercentage[1] + oldSymbolPercentage[2]
						+ oldSymbolPercentage[3]) / 4;
				// If the percentage different is over 40% and more than two characters are
				// missing then the signal has been lost
				if ((av > 40.0) && (missingCharCounter > 2))
					setState(1);
				...
				if (cend == true) {
					...
					// If debugging display the character buffer in binary form + the number of bits
					// since the last character and this baudot character
					if (theApp.isDebug() == true) {
						theApp.writeLine((getCharBuffer() + " (" + Integer.toString(bcount) + ")  " + getBaudotChar()),
								Color.BLACK, theApp.italicFont);
					} else {
					...
					if (bcount > 8)
						missingCharCounter++;
					// If this character and the last character arrived on time clear the mising
					// char counter
					if ((bcount < 9) && (previousbcount < 9))
						missingCharCounter = 0;
				...
				// If the character count has reached MAXCHARLENGTH then display this line and
	...
	private String syncSequenceHunt(CircularDataBuffer circBuf, WaveData waveData) {
		...
		if ((difference < (shift - 25)) || (difference > (shift + 25)))
			return null;
	...
	private String getBaudotChar() {
		...
		if (lettersMode == true)
			return getBAUDOT_LETTERS(a);
		else
			return getBAUDOT_NUMBERS(a);
	}

	// Check if this a valid Baudot character this a start and a stop
	private boolean checkValid15() {
		if ((inChar7[0] == false) && (inChar7[6] == true) && (bcount >= 7))
			return true;
		else
			return false;
	}
	...
	private boolean checkValid20() {
		if ((inChar8[0] == false) && (inChar8[6] == true) && (inChar8[7] == true) && (bcount >= 8))
			return true;
		else
			return false;
	}
	...
	private int rttyFreqHalf(CircularDataBuffer circBuf, WaveData waveData, int pos) {
		...
		if (high1 == high2) {
			...
			// 2.5 stop bits
				...
				// If there isn't a vaid baudot character in the buffer this can't be a half bit
				// and must be a full bit
