...
public class CIS3650 extends FSK {
	...
	public boolean decode(CircularDataBuffer circBuf, WaveData waveData) {
		...
		// Look for a 36 baud or a 50 baud alternating sequence
			...
			if (symbolCounter >= (long) samplesPerSymbol50) {
				...
				addToBuffer7(bit);
				b7Count++;
				// Get 14 bits (to allow the early late gate to settle) but only look at the
				// last 3
				if (b7Count == 14) {
					buffer7 = buffer7 & 0x7;
					...
					if ((buffer7 == 5) || (buffer7 == 2)) {
						...
						if (theApp.isDebug() == true) {
							String dout = theApp.getTimeStamp() + " CIS 36-50 50 baud sync sequence found : lowBin="
									+ Integer.toString(lowBin) + " highBin=" + Integer.toString(highBin);
						...
						if (theApp.isDebug() == true) {
							String dout = theApp.getTimeStamp()
									+ " Unable to obtain CIS 36-50 50 baud alternating sequence";
							theApp.writeLine(dout, Color.BLACK, theApp.italicFont);
						}
						state = 1;
					}
				...
				if (theApp.isDebug() == false) {
					...
					// Read in and display the main body of the message
						...
						if (startCount == 7) {
							...
							totalCharacterCount++;
							// If a message has gone on for 5000 characters there must be a problem so force
							// an end
					...
					// The message must have ended
						...
						String dout = "End of Message (" + Integer.toString(totalCharacterCount)
								+ " characters in this message " + Integer.toString(totalErrorCount)
								+ " of these contained errors)";
						theApp.writeLine(dout, Color.BLACK, theApp.italicFont);
						countSinceSync = 0;
						syncState = 1;
						clearStartBuffer();
	...
	public void setState(int state) {
		...
		if (state == 1)
			...
			theApp.setStatusLabel("Validating Sync");
		else if ((state == 3) && (syncState == 1))
			theApp.setStatusLabel("50 Baud Sync Found");
		else if ((state == 3) && (syncState == 2))
	...
	// Check if the start buffer contains a valid 44 bit sync word and two identical
	// 70 bit session keys
	private boolean checkStartBuffer() {
		int a, count = 0, o;
		// Check for 21 true bits in the first 44 bits
		for (a = 0; a < 44; a++) {
			if (startBuffer[a] == true)
				count++;
		}
		if (count != 21)
			return false;
		...
		for (a = 0; a < 70; a++) {
			if (startBuffer[a + 44] != startBuffer[a + 44 + 70])
				count++;
		}
		if (count > 1)
			return false;
	...
	private int extractIntFromStart(int pos) {
		int v = 0;
		if (startBuffer[pos] == true)
			v = 64;
		if (startBuffer[pos + 1] == true)
			v = v + 32;
		if (startBuffer[pos + 2] == true)
			v = v + 16;
		if (startBuffer[pos + 3] == true)
			v = v + 8;
		if (startBuffer[pos + 4] == true)
			v = v + 4;
		if (startBuffer[pos + 5] == true)
			v = v + 2;
		if (startBuffer[pos + 6] == true)
			v++;
		return v;
	}
