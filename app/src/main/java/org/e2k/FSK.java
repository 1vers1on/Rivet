...
public class FSK extends FFT {
	private final String BAUDOT_LETTERS[] = { "N/A", "E", "<LF>", "A", " ", "S", "I", "U", "<CR>", "D", "R", "J", "N",
			"F", "C", "K", "T", "Z", "L", "W", "H", "Y", "P", "Q", "O", "B", "G", "<FIG>", "M", "X", "V", "<LET>" };
	private final String BAUDOT_NUMBERS[] = { "N/A", "3", "<LF>", "-", " ", "<BELL>", "8", "7", "<CR>", "$", "4", "'",
			",", "!", ":", "(", "5", "+", ")", "2", "#", "6", "0", "1", "9", "?", "&", "<FIG>", ".", "/", "=",
			"<LET>" };
	public final int ITA3VALS[] = { 13, 37, 56, 100, 69, 21, 50, 112, 70, 74, 26, 42, 28, 19, 97, 82, 35, 11, 98, 49,
			22, 76, 73, 25, 84, 81, 67, 88, 38, 14, 41, 44, 52, 104, 7 };
	public final String ITA3LETS[] = { "Q", "W", "E", "R", "T", "Y", "U", "I", "O", "P", "A", "S", "D", "F", "G", "H",
			"J", "K", "L", "Z", "X", "C", "V", "B", "N", "M", "<cr>", "<lf>", "<fig>", "<let>", "<alpha>", "<beta>",
			"<rep>", "<0x68>", "<0x7>" };
	public final int CCIR476VALS[] = { 106, 92, 46, 39, 86, 85, 116, 43, 78, 77, 113, 45, 71, 75, 83, 27, 53, 105, 23,
			30, 101, 99, 58, 29, 60, 114, 89, 57, 120, 108, 54, 90, 15 };
	public final String CCIR476LETS[] = { "<32>", " ", "Q", "W", "E", "R", "T", "Y", "U", "I", "O", "P", "A", "S", "D",
			"F", "G", "H", "J", "K", "L", "Z", "X", "C", "V", "B", "N", "M", "<cr>", "<lf>", "<fig>", "<let>",
			"<alpha>" };
	public final String CCIR476NUMS[] = { "<32>", " ", "1", "2", "3", "4", "5", "6", "7", "8", "9", "0", "-", "'", " ",
			"%", "@", "#", "*", "(", ")", "+", "/", ":", "=", "?", ",", ".", "<cr>", "<lf>", "<fig>", "<let>",
			"<alpha>" };
	...
	public int doRTTY_8000FFT(CircularDataBuffer circBuf, WaveData waveData, int start, int ss, double baud) {
		...
		// 100 baud
			...
			return (do100baudFFT(circBuf, waveData, start));
		// 145 baud
		else if (baud == 145)
			return (do145baudFFT(circBuf, waveData, start));
		// 150 baud
		else if (baud == 150)
			return (do150baudFFT(circBuf, waveData, start));
		// 200 baud
		else if (baud == 200)
			return (doFSK200500_8000FFT(circBuf, waveData, start, ss));
		// 300 baud
		else if (baud == 300)
			return (do300baudFFT(circBuf, waveData, start));
		// 600 baud
		else if (baud == 600)
			return (do600baudFFT(circBuf, waveData, start));
		else
			return 0;
	}
	...
	// Does a 80 point FFT on 40 samples (a half symbol) then returns the values of
	// two specific bins
	...
	// Does a 80 point FFT on 27 samples (a half symbol) then returns the values of
	// two specific bins
	...
	// Does a 80 point FFT on 26 samples (a half symbol) then returns the values of
	// two specific bins
	...
	// Does a 64 point FFT on 20 samples (a half symbol) then returns the values of
	// two specific bins
	...
	// Does a 64 point FFT on 13 samples (a half symbol) then returns the values of
	// two specific bins
	...
	// Does a 64 point FFT on 6 samples (a half symbol) then returns the values of
	// two specific bins
	...
	public double[] doRTTYHalfSymbolBinRequest(double baud, CircularDataBuffer circBuf, int start, int bin0, int bin1) {
		...
		// 100 baud
			...
			return (do100baudFSKHalfSymbolBinRequest(circBuf, start, bin0, bin1));
		// 145 baud
		else if (baud == 145)
			return (do145baudFSKHalfSymbolBinRequest(circBuf, start, bin0, bin1));
		// 150 baud
		else if (baud == 150)
			return (do150baudFSKHalfSymbolBinRequest(circBuf, start, bin0, bin1));
		// 200 baud
		else if (baud == 200)
			return (do200baudFSKHalfSymbolBinRequest(circBuf, start, bin0, bin1));
		// 300 baud
		else if (baud == 300)
			return (do300baudFSKHalfSymbolBinRequest(circBuf, start, bin0, bin1));
		// 600 baud
		else if (baud == 600)
	...
	public boolean toneTest(int freq, int tone, int errorAllow) {
		if ((freq > (tone - errorAllow)) && (freq < (tone + errorAllow)))
			return true;
		else
			return false;
	}
	...
	public double[] do64FFTHalfSymbolBinRequest(CircularDataBuffer circBuf, int start, int samples, int bin0,
			int bin1) {
		double vals[] = new double[2];
		int a;
		double datar[] = new double[FFT_64_SIZE];
