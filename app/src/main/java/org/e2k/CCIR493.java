...
public class CCIR493 extends FSK {
	...
	private final int VALIDWORDS[] = { 7, 518, 262, 773, 134, 645, 389, 900, 70, 581, 325, 836, 197, 708, 452, 963, 38,
			549, 293, 804,
	...
	private boolean getSymbolFreqBin(CircularDataBuffer circBuf, WaveData waveData, int start) {
		...
		if (lowTotal > highTotal)
			kalmanFilter(getPercentageDifference(early[0], late[0]), KALMAN1, KALMAN2);
		else
			kalmanFilter(getPercentageDifference(early[1], late[1]), KALMAN1, KALMAN2);
	...
	private void handleTraffic(boolean b) {
		...
		if (messageState == 0) {
			...
			if (buffer10 == 262) {
				if (theApp.isDebug() == true)
					outLines[0] = theApp.getTimeStamp() + " CCIR493-4 Inverted PDX";
				invertedPDXCounter++;
			}
			...
			if (c == 125) {
				if (theApp.isDebug() == true)
					outLines[0] = theApp.getTimeStamp() + " CCIR493-4 PDX";
				dx++;
			} else if ((c <= 111) && (c >= 104)) {
				if (theApp.isDebug() == true)
					outLines[0] = theApp.getTimeStamp() + " CCIR493-4 RX (" + Integer.toString(c) + ")";
				rx++;
			}
			// Is phasing complete ?
			if (((dx == 2) && (rx == 1)) || ((dx == 1) && (rx == 2))) {
				...
				messageState = 1;
				if (theApp.isDebug() == true)
					outLines[0] = theApp.getTimeStamp() + " CCIR493-4 Phasing Detect : PDX=" + Integer.toString(dx)
							+ " RX=" + Integer.toString(rx);
			}
			if (bitCount > 1800) {
				if (theApp.isDebug() == true)
					outLines[0] = theApp.getTimeStamp() + " CCIR493-4 Phasing Timeout";
				state = 1;
			}
		}
		// Phasing complete now look for the format specifier
			...
			if (bitCount % 10 == 0) {
				...
				if (theApp.isDebug() == true) {
					if (c2 != -1)
						outLines[0] = theApp.getTimeStamp() + " CCIR493-4 Character " + Integer.toString(c2);
					else
						outLines[0] = theApp.getTimeStamp() + " CCIR493-4 Character Error was "
								+ Integer.toString(unCorrectedInput);
				}
				// If we haven't received a format specifier after 300 bits then something has
				// gone wrong
				if (bitCount > 300) {
					if (theApp.isDebug() == true)
						outLines[0] = theApp.getTimeStamp() + " CCIR493-4 Format Specifier Timeout";
					state = 1;
				}
				if (formatSpecifier != -1) {
					bitCount = 0;
					messageState = 2;
					if (formatSpecifier == 112)
						lineBuffer.append(theApp.getTimeStamp() + " CCIR493-4 Distress Alert ");
					else if (formatSpecifier == 116)
						lineBuffer.append(theApp.getTimeStamp() + " CCIR493-4 All Stations ");
					else if (formatSpecifier == 114)
						lineBuffer.append(theApp.getTimeStamp() + " CCIR493-4 Group Selective Call ");
					else if (formatSpecifier == 120)
						lineBuffer.append(theApp.getTimeStamp() + " CCIR493-4 Individual Selective Call ");
					else if (formatSpecifier == 102)
						lineBuffer.append(theApp.getTimeStamp() + " CCIR493-4 Geographic Selective Call ");
					else if (formatSpecifier == 123)
						lineBuffer.append(theApp.getTimeStamp()
								+ " CCIR493-4 Individual Selective Call Using Semi/Automatic Service ");
					else
						lineBuffer.append(theApp.getTimeStamp() + " CCIR493-4 Unknown Call ");
				}
			}
		}
		// Load the body of the message into the messageBuffer array
			...
			if (bitCount % 10 == 0) {
				...
				if (i >= messageBuffer.length)
					...
					messageBuffer[i - 1] = ret10BitCode(buffer10, true);
				// Check for an ARQ ARQ end sequence
				if (i > 3) {
					if ((messageBuffer[i - 1] == 117) && (messageBuffer[i - 2] == 117))
						messageState = 3;
				}
			}
		}
		...
		for (a = 0; a < outLines.length; a++) {
			if (outLines[a] != null)
				theApp.writeLine(outLines[a], Color.BLACK, theApp.boldFont);
		}
		return;
	}
	...
	// Returns a 7 bit value from a 10 bit block the last 3 bits give the number of
	// B (0) bits in the first 7 bits
	// if there is an error then -1 is returned the routine will fix words with up
	// to errorMax bits incorrect
	...
	private int ret10BitCode(int in, boolean errorBitsAllowed) {
		...
		for (a = 0; a < VALIDWORDS.length; a++) {
			...
			for (b = 0; b < BITVALUES.length; b++) {
				if ((in & BITVALUES[b]) != (VALIDWORDS[a] & BITVALUES[b]))
					dif++;
			}
			if (dif <= errorMax)
				return a;
	...
	private String[] decodeMessageBody() {
		...
		ol[0] = lineBuffer.toString() + "Station " + getAStationIdentity() + " Calling " + getBStationIdentity()
				+ getCategory();
		return ol;
	}
