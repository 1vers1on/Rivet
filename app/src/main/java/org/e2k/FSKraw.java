...
public class FSKraw extends FSK {
	...
	public boolean decode(CircularDataBuffer circBuf, WaveData waveData) {
		...
		// Decode traffic
			...
			if (symbolCounter >= samplesPerSymbol) {
				...
				if (theApp.getActiveTriggerCount() > 0)
					triggerCheck();
				else
					display = true;
				// Display this but only if we are still in state 2 (to prevent any duff binary
				// appearing)
	...
	private void addToAdjBuffer(double in) {
		...
		if (NOISY)
			lossAverage = 60.0;
		// If the buffer average percentage difference is more than lossAverage then we
