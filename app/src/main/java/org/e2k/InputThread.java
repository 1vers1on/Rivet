...
public class InputThread extends Thread {
	...
	public void run() {
		// Run continuously
		for (;;) {
			// If the audio device is ready , the program wants to and we aren't already
			// then
			// get data from the audio device.
			if ((audioReady == true) && (loadingFile == false) && (gettingAudio == false))
				getSample();
	...
	private boolean grabWavBlock() {
		...
		// 16 bit LE
		if ((audioInputStream.getFormat().isBigEndian() == false)
				&& (audioInputStream.getFormat().getSampleSizeInBits() == 16)) {
			return grabWavBlock16LE(audioInputStream);
		}
		// 8 bit LE
		else if ((audioInputStream.getFormat().isBigEndian() == false)
	...
	public void setupAudio(WaveData waveData) {
		try {
			...
			AudioFormat format = new AudioFormat((int) waveData.getSampleRate(), waveData.getSampleSizeInBits(),
					waveData.getChannels(), true, waveData.isEndian());
	...
	// Read in 2 bytes from the audio source combine them together into a single
	// integer
